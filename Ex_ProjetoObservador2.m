...
% Projeto de observador de estados: preditor x filtro
%
...
for k = 2:N
    ...
    u1(k) = -K*xhat1(:,k);
    % Processo controlado:
    xmf1(:,k+1) = A*xmf1(:,k) + B*u1(k); % Observe que o modelo do mesmo processo usado nesse projeto nao e o mesmo dos outros casos
    ymf1(:,k+1) = C*xmf1(:,k+1);
    ...
    u2(k) = -K*xhat2(:,k);
    % Processo controlado:
    xmf2(:,k+1) = A*xmf2(:,k) + B*u2(k); % Observe que o modelo do mesmo processo usado nesse projeto nao e o mesmo dos outros casos
    ymf2(:,k+1) = C*xmf2(:,k+1);
end
figure; subplot(311); stairs(kT, xmf1(1,1:end-1), 'b'); hold on; stairs(kT, xhat1(1,:), 'r'); stairs(kT, xhat2(1,:), 'm'); xlabel('kT (s)'); ylabel('x_1(k)'); legend('x_1(k)','xhat_1(k)','xhat_2(k)')
        subplot(312); stairs(kT, xmf1(2,1:end-1), 'b'); hold on; stairs(kT, xhat1(2,:), 'r'); stairs(kT, xhat2(2,:), 'm'); xlabel('kT (s)'); ylabel('x_2(k)');
        subplot(313); stairs(kT, u1, 'r'); hold on; stairs(kT, u2, 'm'); xlabel('kT (s)'); ylabel('u(k)');
